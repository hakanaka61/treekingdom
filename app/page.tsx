...
// ==========================================
// 1. AYARLAR & OYUN DENGESİ (V28.0 PERFORMANCE)
// ==========================================
const CONFIG = {
  TILE_WIDTH: 128, TILE_HEIGHT: 64,
  MAP_SIZE: 60, // Performans için 60'a optimize edildi
...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    const savedUid = localStorage.getItem("orman_v28_uid");
      ...
      try {
          ...
          if (snapshot.exists()) {
              ...
              if (foundData && foundData.player.pin === pinInput) {
                  localStorage.setItem("orman_v28_uid", foundUid!);
              ...
              localStorage.setItem("orman_v28_uid", newUid); setLoginModal(false); initWorld(newUid);
          }
  ...
  const spawnUnit = (type: 'worker' | 'king' | 'upgrade_king' | 'soldier') => {
      ...
      if(type==='king' && gs.current.entities.find(e => e.type==='king' && e.owner === gs.current.userId && !e.dead)) { 
  ...
  const updateLogic = () => {
                  ...
                  if(ent.state === 'IDLE' && !ent.targetId) {
                      ...
                      if(t && t.hp > 0) {
                          ...
                          if(Date.now() - ent.workStartTime >= requiredTime) {
                              ...
                              t.dead = true; // HEDEFİ ÖLÜ İŞARETLE
      ...
      // ÖLÜLERİ TEMİZLE (DONMA ENGELLEYİCİ)
