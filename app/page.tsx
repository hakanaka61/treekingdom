...
// ==========================================
// 1. AYARLAR & OYUN DENGESİ (V30.0 ALIVE)
// ==========================================
const CONFIG = {
  TILE_WIDTH: 128, TILE_HEIGHT: 64,
  MAP_SIZE: 80, // Harita 80x80 (Sabit)
  ...
  NIGHT_DURATION_RATIO: 0.2, // %20'si Gece (1dk)
  MANA_REGEN_RATE: 0.5, 
  BASE_STORAGE: 500,
  ...
  TRADES: [
      { id: 1, give: { type: 'wood', amount: 200 }, get: { type: 'gold', amount: 20 }, desc: "Odun Sat" },
      { id: 2, give: { type: 'stone', amount: 100 }, get: { type: 'gold', amount: 50 }, desc: "Taş Sat" },
      { id: 3, give: { type: 'gold', amount: 100 }, get: { type: 'wood', amount: 500 }, desc: "Odun Al" }
  ],

  // TÜCCAR FIRSATLARI (Daha ucuz)
  TRADER_DEALS: [
  ...
  ASSETS: {
    ...
    soldier: '/assets/soldier.png', barbarian: '/assets/barbarian.png', trader: '/assets/trader.png' // Tüccar eklendi
  }
};
...
export default function GamePage() {
  ...
  const gs = useRef({
    ...
    nightMode: false, weather: 'sunny', // 'sunny' | 'rain'
    timerText: "00:00", traderActive: false
  });
  ...
  const spawnRandomResource = (initial = false) => {
      ...
      while(!found && attempt < 50) {
          ...
          if(d(rx,ry,cx,cx) < CONFIG.MAP_SIZE/2 - 3 && (Math.abs(rx-cx)>3 || Math.abs(ry-cx)>3)) {
  ...
  // --- AUTO-REPAIR SİSTEMİ (ESKİ HESAPLARI KURTARIR) ---
  const checkAndRepairMap = (val: any) => {
      // Eğer sis haritası yoksa veya boyutu yanlışsa, yeniden oluştur.
  ...
  const connectToDb = (uid: string) => {
      onValue(ref(db, `empires_final/${uid}`), (snap) => {
          ...
          if(val) {
              ...
              checkAndRepairMap(val); // TAMİR ET
              updateUi();
          } else { initWorld(uid); }
      });
  };
  ...
  useEffect(() => {
    ...
    const savedUid = localStorage.getItem("orman_v30_uid");
      ...
      try {
          ...
          if (snapshot.exists()) {
              ...
              if (foundData && foundData.player.pin === pinInput) {
                  localStorage.setItem("orman_v30_uid", foundUid!);
              ...
              localStorage.setItem("orman_v30_uid", newUid); setLoginModal(false); initWorld(newUid);
          }
  ...
  const executeTrade = (tradeId: number) => {
      // Normal Takas
      let trade = CONFIG.TRADES.find(t => t.id === tradeId);
      // Tüccar Takası
      if(!trade) trade = CONFIG.TRADER_DEALS.find(t => t.id === tradeId);
  ...
  const updateLogic = () => {
      ...
      const isNight = cycle > (1 - CONFIG.NIGHT_DURATION_RATIO); // Son %20'lik kısım gece (1 dk)
      gs.current.nightMode = isNight;
      gs.current.timeOfDay = isNight ? 0.6 : 0;

      // HAVA DURUMU (Rastgele Yağmur)
      if(Math.random() < 0.001) gs.current.weather = gs.current.weather === 'sunny' ? 'rain' : 'sunny';

      // TÜCCAR (Sadece Gündüz)
      if(!isNight && Math.random() < 0.0005 && !gs.current.traderActive) {
          gs.current.traderActive = true;
          spawnFloatingText(0,0, "TÜCCAR GELDİ!", "#d946ef");
          setTimeout(() => { gs.current.traderActive = false; spawnFloatingText(0,0, "Tüccar Gitti...", "#d946ef"); updateUi(); }, 30000); // 30sn Kalır
          updateUi();
      }
      ...
      const remainingSeconds = isNight 
          ...
          : Math.floor(((1 - CONFIG.NIGHT_DURATION_RATIO) - cycle) * totalSeconds);
      const minutes = Math.floor(remainingSeconds / 60);
      const seconds = remainingSeconds % 60;
      gs.current.timerText = `${isNight ? 'Kanlı Ay' : 'Gündüz'} ${minutes}:${seconds < 10 ? '0'+seconds : seconds}`;

      if(isNight && now - gs.current.lastEnemySpawn > 10000) { 
          const myUnits = gs.current.entities.filter(e => (e.type === 'worker' || e.type === 'soldier' || e.type === 'king') && e.owner === gs.current.userId && !e.dead).length;
          if(myUnits >= 3) {
              const soldiers = gs.current.entities.filter(e => e.type === 'soldier' && e.owner === gs.current.userId && !e.dead).length;
      ...
      if(now - gs.current.lastIncomeTime > 10000) {
          ...
          let farmBonus = gs.current.weather === 'rain' ? 2 : 1; // YAĞMURDA 2 KAT VERİM
      ...
      const resourceCount = gs.current.entities.filter(e => e.owner === 'nature' && !e.dead).length;
      ...
      // YAĞMUR EFEKTİ
      if(gs.current.weather === 'rain') {
          if(Math.random() < 0.3) {
             const cx = Math.floor(CONFIG.MAP_SIZE/2) + (Math.random()-0.5)*20;
             const cy = Math.floor(CONFIG.MAP_SIZE/2) + (Math.random()-0.5)*20;
             spawnParticle(cx, cy, "#3b82f6");
          }
      }

      gs.current.particles.forEach(p => { p.y -= p.velocityY; p.life--; }); gs.current.particles = gs.current.particles.filter(p => p.life > 0);
      const king = gs.current.entities.find(e => e.type === 'king' && e.owner === gs.current.userId && !e.dead);
      ...
      gs.current.entities.forEach(ent => {
          ...
          if(ent.type === 'barbarian') {
              ...
              gs.current.entities.forEach(t => {
                  if(!t.dead && t.owner === gs.current.userId && t.type !== 'barbarian' && t.type !== 'castle') {
              ...
              if(target && minD < 1) { 
                  if(Math.random() < 0.05 && !gs.current.shieldActive) { 
                      ...
                      spawnParticle(target.pos.x, target.pos.y, "red"); // KAN EFEKTİ
                      if(target.hp <= 0) {
                          target.dead = true;
          ...
          if(ent.type === 'soldier') {
              let target: any = gs.current.entities.find(e => !e.dead && e.type === 'barbarian');
              if(target) {
                  ...
                  if(!ent.patrolTarget || Math.random() < 0.01) {
                      const angle = Math.random() * Math.PI * 2;
                      ent.patrolTarget = {x: ent.pos.x + Math.cos(angle)*5, y: ent.pos.y + Math.sin(angle)*5};
                  }
                  const d = Math.hypot(ent.patrolTarget.x - ent.pos.x, ent.patrolTarget.y - ent.pos.y);
                  if(d > 0.1) {
                      const dx = ent.patrolTarget.x - ent.pos.x; const dy = ent.patrolTarget.y - ent.pos.y;
                      ent.pos.x += (dx/d)*0.02; ent.pos.y += (dy/d)*0.02;
                  }
              }
          }
              ...
              if(ent.type === 'worker' || ent.type === 'king') {
                  ...
                  if(ent.state === 'IDLE' && !ent.targetId) {
                      ...
                      gs.current.entities.forEach(e => { 
                          if(!e.dead && (e.type==='tree'||e.type==='stone'||e.type==='gold'||e.type==='deer'||e.type==='chest') && e.hp>0) { 
                      ...
                      let tx, ty, dist;
                      if(ent.targetId) { const t = gs.current.entities.find(e => e.id === ent.targetId && !e.dead); if(t) { tx=t.pos.x; ty=t.pos.y; } else { ent.state='IDLE'; ent.targetId=null; return; } } else { tx=ent.targetPos.x; ty=ent.targetPos.y; }
                      ...
                      const t = gs.current.entities.find(e => e.id === ent.targetId && !e.dead);
                      if(t && t.hp > 0) {
                          ...
                          if(Date.now() - ent.workStartTime >= requiredTime) {
                              ...
                              if(t.type==='tree') { if(currentRes.wood < cap) { currentRes.wood += val; gs.current.player.stats.totalWood += val; updateQuest('wood', val); spawnFloatingText(ent.pixelPos.x, ent.pixelPos.y - 50, `+${val} Odun`, '#a3e635'); spawnParticle(ent.pos.x, ent.pos.y, "#a3e635"); } else spawnFloatingText(ent.pixelPos.x, ent.pixelPos.y - 50, "DEPO DOLU!", "red"); }
                              if(t.type==='stone') { if(currentRes.stone < cap) { currentRes.stone += val; updateQuest('stone', val); spawnFloatingText(ent.pixelPos.x, ent.pixelPos.y - 50, `+${val} Taş`, '#94a3b8'); spawnParticle(ent.pos.x, ent.pos.y, "#94a3b8"); } }
                              ...
                              t.dead = true; 
                              ent.state='IDLE'; ent.targetId=null; updateUi(); saveGame();
                          }
    ...
    if(e.type==='mousedown' || e.type==='touchstart') { gs.current.input.isDragging = false; gs.current.input.canClick = true; gs.current.input.startX = cx; gs.current.input.startY = cy; gs.current.input.lastX = cx; gs.current.input.lastY = cy; } 
        ...
        if(e.type === 'touchmove' || e.buttons === 1) {
             ...
             if(gs.current.input.isDragging) { 
                 const deltaX = (gs.current.input.lastX - cx) * 1.5;
                 const deltaY = (gs.current.input.lastY - cy) * 1.5;
                 if(!isNaN(deltaX)) gs.current.camera.targetX += deltaX; 
                 if(!isNaN(deltaY)) gs.current.camera.targetY += deltaY; 
             }
        ...
        if(gs.current.input.canClick && !buildMode) {
             ...
             const clickedEnt = gs.current.entities.find(en => !en.dead && en.pos.x === ix && en.pos.y === iy);
             if(!clickedEnt) { const king = gs.current.entities.find(k => k.type === 'king' && k.owner === gs.current.userId && !k.dead); if(king) { king.state = 'MOVE'; king.targetPos = {x:ix, y:iy}; king.targetId = null; setInfoText("Kral Yürüyor..."); } }
  ...
  const render = () => {
    ...
    gs.current.entities.forEach(ent => {
        if(ent.dead || !gs.current.fog[Math.floor(ent.pos.x)][Math.floor(ent.pos.y)]) return;
        ...
        if(img && img.complete) {
            ...
            // EMOJI (DURUM) BALONLARI
            if(ent.type === 'worker' && ent.state === 'IDLE') { ctx.font = `${12*zoom}px Arial`; ctx.fillText("💤", pos.x, drawY); }
            if(ent.type === 'worker' && ent.state === 'WORK') { ctx.font = `${12*zoom}px Arial`; ctx.fillText("🔨", pos.x, drawY); }
            if(ent.type === 'soldier') { ctx.font = `${12*zoom}px Arial`; ctx.fillText("⚔️", pos.x, drawY); }
    ...
    if(gs.current.nightMode) { ctx.fillStyle = 'rgba(60, 0, 0, 0.3)'; ctx.fillRect(0,0,cvs.width, cvs.height); } // KANLI AY EFEKTİ
    gs.current.particles.forEach(p => { const screenX = (p.x * zoom) + cvs.width/2 - cam.x; const screenY = (p.y * zoom) + cvs.height/2 - cam.y; 
        if(p.isDot) { ctx.fillStyle = p.color; ctx.fillRect(screenX, screenY, 4*zoom, 4*zoom); } 
        else { ctx.globalAlpha = p.life / p.maxLife; ctx.fillStyle = p.color; ctx.font = `bold ${16 * zoom}px Arial`; ctx.strokeText(p.text, screenX, screenY); ctx.fillText(p.text, screenX, screenY); ctx.globalAlpha = 1.0; }
    });
  };
    ...
    <div className="fixed inset-0 bg-gray-900 text-white select-none overflow-hidden touch-none font-sans touch-none">
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-slate-900 rounded-2xl border border-yellow-500/30 w-full max-w-lg p-6 shadow-2xl overflow-y-auto max-h-[80vh]">
                ...
                <div className="space-y-4 text-xs text-gray-300">
                    <p>Hava Durumu: {ui.weather === 'rain' ? '🌧️ Yağmurlu (Çiftlik x2)' : '☀️ Güneşli'}</p>
                    <p>Tüccar: {ui.trader ? '✅ Aktif (Mor Adamı Bul)' : '❌ Yok'}</p>
                </div>
            </div>
        </div>
      )}
      ...
      <div className="absolute top-4 left-0 right-0 z-20 flex justify-center px-4"> 
          <div className="flex flex-col items-center gap-1">
             <div className={`px-3 py-1 rounded-full border text-[10px] font-mono ${ui.isNight ? 'bg-red-900/80 border-red-500 text-red-200' : 'bg-black/80 border-white/20 text-gray-300'}`}>
                 🕒 {ui.timer} {ui.weather==='rain' && '🌧️'}
             </div>
              ...
              <div className="bg-purple-900 rounded-2xl border border-purple-500 w-72 p-6 shadow-2xl">
                  ...
                  {CONFIG.TRADER_DEALS.map(t => (
                      <button key={t.id} onClick={()=>executeTrade(t.id)} className="w-full bg-purple-800 p-3 rounded mb-2 border border-purple-600 hover:bg-purple-700 text-left">
                          ...
                          <div className="text-xs text-purple-200">Ver: {t.give.amount} {t.give.type} -> Al: {t.get.amount} {t.get.type}</div>
                      </button>
                  ))}
