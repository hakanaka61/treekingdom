...
// ==========================================
// 1. AYARLAR & OYUN DENGESİ (V26.0 STABLE)
// ==========================================
const CONFIG = {
  ...
  MIN_WORK_TIME: 2000,    
  DAY_CYCLE_DURATION: 300000, // 5 DAKİKA (300.000 ms)
  MANA_REGEN_RATE: 0.5, 
  BASE_STORAGE: 500,
...
export default function GamePage() {
  ...
  const [showHelp, setShowHelp] = useState(false); // Yeni: Yardım Kutusu
  ...
  useEffect(() => {
    ...
    const savedUid = localStorage.getItem("orman_v26_uid");
      ...
      try {
          ...
          if (snapshot.exists()) {
              ...
              if (foundData && foundData.player.pin === pinInput) {
                  localStorage.setItem("orman_v26_uid", foundUid!);
              ...
              localStorage.setItem("orman_v26_uid", newUid); setLoginModal(false); initWorld(newUid);
          }
  ...
  const spawnUnit = (type: 'worker' | 'king' | 'upgrade_king' | 'soldier') => {
      ...
      if(type === 'upgrade_king') {
          ...
          const upgradeCost = currentLevel * 500; // Örn: Lvl 1 -> Lvl 2 için 500 altın
      ...
      if(gs.current.player.resources.food >= costFood && gs.current.player.resources.gold >= costGold && gs.current.player.resources.stone >= costStone) {
          ...
          // Hangi kaynağın eksik olduğunu söyle
  ...
  const updateLogic = () => {
      ...
      // BARBAR SPAWN (Sadece Gece, 10sn aralıkla ve NÜFUS > 3 ise)
      if(isNight && now - gs.current.lastEnemySpawn > 10000) { 
          ...
          // MERHAMET KURALI: 3 birimden azsa barbar gelmez
          if(myUnits >= 3) {
      ...
      gs.current.entities.forEach(ent => {
          ...
          // ASKER DONMA FIX: Hedef kontrolü güvenli hale getirildi
          if(ent.type === 'soldier') {
