...
const CONFIG = {
  ...
  BASE_SPAWN_TIME: 25000, // Spawn süresi biraz hızlandırıldı
  BASE_WORK_TIME: 10000,  
  MIN_WORK_TIME: 2000,    
  DAY_CYCLE_DURATION: 60000,
  MANA_REGEN_RATE: 2, // Mana dolumu hızlandırıldı

  // ORANLAR (Geyik patlaması)
  SPAWN_RATES: {
      tree: 0.35,   // Ağaç azaltıldı
      stone: 0.20,  // Taş sabit
      deer: 0.35,   // %35 GEYİK (BÜYÜK ARTIŞ)
      gold: 0.08,   // Altın
      chest: 0.02   // Sandık
  },
...
export default function GamePage() {
  ...
  const gs = useRef({
    ...
    input: { isDragging: false, startX: 0, startY: 0, lastX: 0, lastY: 0 }, // Drag fix için güncellendi
  ...
  const [showTutorial, setShowTutorial] = useState(false); // Yeni Tutorial State'i
  ...
  const initWorld = (uid: string) => {
      ...
      saveGame();
      setShowTutorial(true); // Yeni hesap açınca rehberi göster
  };
  ...
  const castSpell = (spell: 'speed' | 'nature') => {
      ...
      if(spell === 'nature') {
          if(p.mana >= 80) {
              p.mana -= 80;
              for(let i=0; i<15; i++) spawnRandomResource(); // Büyü daha güçlü
  ...
  const handleInput = (e: any) => {
    ...
    if(e.type==='mousedown' || e.type==='touchstart') {
        ...
        // Eğer sürükleme olmadıysa (yani sadece tıklandıysa)
