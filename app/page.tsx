...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    const savedUid = localStorage.getItem("orman_v18_uid");
    if (savedUid) {
      ...
      try {
          ...
          if (snapshot.exists()) {
              ...
              if (foundData && foundData.player.pin === pinInput) {
                  localStorage.setItem("orman_v18_uid", foundUid!);
              ...
              localStorage.setItem("orman_v18_uid", newUid);
              setLoginModal(false);
              initWorld(newUid);
  ...
  const castSpell = (spell: 'speed' | 'nature') => {
      ...
      if(spell === 'nature') {
          if(p.mana >= 80) {
              p.mana -= 80;
              for(let i=0; i<10; i++) spawnRandomResource();
  ...
  const updateQuest = (type: string, amount: number) => {
      ...
      if(q && q.active && q.type === type) {
          ...
          if(q.current >= q.target) {
              q.active = false;
              gs.current.player.resources.gold += 100; // Bonus ödül
              gs.current.player.stats.score += q.reward;
              spawnFloatingText(0, 0, "GÖREV TAMAMLANDI!", "#22c55e");
              log("Günlük Görev Tamamlandı: +1000 Puan");
          }
      }
  };
  ...
  const saveGame = () => {
      ...
      score += (p.achievements.length * 1000); // Başarım başına puan
  ...
  const handleInput = (e: any) => {
    ...
    if(e.type==='mousedown' || e.type==='touchstart') {
        ...
        if(!buildMode) {
             ...
             const mcy = (e.clientY || e.touches?.[0]?.clientY) - rect.top;
        ...
        } else if(buildMode && e.type!=='touchstart') handleBuild(e);
    } 
