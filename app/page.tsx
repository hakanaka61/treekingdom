...
// ==========================================
// 1. AYARLAR & OYUN DENGESİ (V25.0 LIVE)
// ==========================================
const CONFIG = {
  TILE_WIDTH: 128, TILE_HEIGHT: 64,
  MAP_SIZE: 80, // Harita Büyütüldü
  ZOOM_MIN: 0.2, ZOOM_MAX: 1.5,
  OVERLAP_STRENGTH: 1.20,
  
  BASE_SPAWN_TIME: 25000, 
  FAST_SPAWN_TIME: 3000, // Kaynak azken hızlanır
  BASE_WORK_TIME: 10000,  
  MIN_WORK_TIME: 2000,    
  DAY_CYCLE_DURATION: 60000, // 30sn Gece / 30sn Gündüz
  MANA_REGEN_RATE: 0.5, 
  BASE_STORAGE: 500,
...
export default function GamePage() {
      ...
      while(!found && attempt < 50) {
          ...
          if(d < CONFIG.MAP_SIZE/2 - 3 && (Math.abs(rx-cx)>3 || Math.abs(ry-cx)>3)) {
              if(!gs.current.entities.find(e => e.pos.x === rx && e.pos.y === ry)) {
                   ...
                   // SU KENARI MANTIĞI (Ağaç yoğunluğu)
                   let nearWater = false;
                   ...
                   if(nearWater && Math.random() < 0.8) type = 'tree'; // Su kenarına ağaç dik
  ...
  const updateLogic = () => {
      ...
      // ADAPTİF BARBAR SİSTEMİ (ASKER SAYISINA GÖRE)
      if(isNight && now - gs.current.lastEnemySpawn > 10000) { // Her 10 sn
      ...
      gs.current.entities.forEach(ent => {
          ...
          if(ent.type === 'barbarian') {
              ...
              gs.current.entities.forEach(t => {
                  // KALE HEDEF ALINAMAZ (castle hariç tutuldu)
              ...
              if(target) {
                  ...
                  if(d < 1) {
                      if(Math.random() < 0.1) {
                          ...
                          target.hp -= 20 + (warLvl * 5); // SAVAŞ SANATI BONUSU
          ...
          if(ent.owner === gs.current.userId) {
              if(ent.type === 'worker' || ent.type === 'king') {
                  ...
                  if(ent.state === 'IDLE' && !ent.targetId) {
                      ...
                      // AKILLI İŞÇİ (YAKIN KAYNAK ÖNCELİĞİ)
  ...
  const handleBuild = (e: any) => {
      ...
      if(gs.current.player.resources[resType] >= building.cost) {
          ...
          if(buildMode === 'house') gs.current.player.maxPop += 5; if(buildMode === 'storage') gs.current.player.storageCap += 1000;
  ...
  const spawnUnit = (type: 'worker' | 'king' | 'upgrade_king' | 'soldier') => {
      ...
      if(type === 'upgrade_king') {
          const currentLevel = gs.current.player.heroLevel || 1; const upgradeCost = currentLevel * 500; 
          if(gs.current.player.resources.gold >= upgradeCost) { gs.current.player.resources.gold -= upgradeCost; gs.current.player.heroLevel = currentLevel + 1; setInfoText(`Kral Seviye ${currentLevel + 1} Oldu!`); spawnFloatingText(0,0, "LEVEL UP!", "#facc15"); updateUi(); saveGame(); } else { setInfoText(`Yetersiz Altın (${upgradeCost} Gerekli)`); }
          return;
      }
      const costFood = type==='worker' ? 60 : 0; const costGold = (type==='king' ? 500 : (type==='soldier' ? 50 : 0)); const costStone = type==='soldier' ? 20 : 0;
      ...
      if(gs.current.player.resources.food >= costFood && gs.current.player.resources.gold >= costGold && gs.current.player.resources.stone >= costStone) {
          ...
          updateUi(); saveGame(); 
          if(type==='worker') spawnFloatingText(0, 0, "-60 Et", 'red'); if(type==='soldier') spawnFloatingText(0, 0, "ASKER HAZIR!", 'gray'); if(type==='king') { gs.current.player.heroLevel = 1; spawnFloatingText(0, 0, "KRAL GELDİ!", '#facc15'); }
      } else { setInfoText(`Yetersiz Kaynak`); }
  };

  const handleZoom = (d: number) => { let z = gs.current.camera.zoom + d; gs.current.camera.zoom = Math.max(CONFIG.ZOOM_MIN, Math.min(CONFIG.ZOOM_MAX, z)); };
    ...
    <div className="fixed inset-0 bg-gray-900 text-white select-none overflow-hidden touch-none font-sans touch-none">
      ...
      {activeMenu === 'magic' && (
          <div className="absolute bottom-20 left-1/2 -translate-x-1/2 bg-black/90 p-4 rounded-xl border border-blue-500/50 w-72 backdrop-blur z-30 animate-in slide-in-from-bottom-5">
              ...
              <button onClick={()=>castSpell('nature')} className="w-full bg-slate-800 p-2 rounded mb-2 flex justify-between items-center hover:bg-green-900/30"><div><div className="font-bold text-sm">🌱 Doğa Ana</div><div className="text-[10px] text-gray-400">Anında Kaynak Çıkarır</div></div><span className="text-green-300 font-mono">80 Mana</span></button>
              <button onClick={()=>castSpell('shield')} className="w-full bg-slate-800 p-2 rounded mb-2 flex justify-between items-center hover:bg-cyan-900/30"><div><div className="font-bold text-sm">🛡️ Kutsal Kalkan</div><div className="text-[10px] text-gray-400">15sn Hasar Alınmaz</div></div><span className="text-cyan-300 font-mono">100 Mana</span></button>
              <button onClick={()=>castSpell('goldrain')} className="w-full bg-slate-800 p-2 rounded flex justify-between items-center hover:bg-yellow-900/30"><div><div className="font-bold text-sm">💰 Altın Yağmuru</div><div className="text-[10px] text-gray-400">+100 Altın</div></div><span className="text-yellow-300 font-mono">80 Mana</span></button>
          </div>
      )}
      {activeMenu === 'build' && (
          <div className="absolute bottom-20 left-1/2 -translate-x-1/2 bg-black/90 p-4 rounded-xl border border-orange-500/50 w-72 backdrop-blur z-30 animate-in slide-in-from-bottom-5">
      ...
      {activeMenu === 'market' && (
          <div className="absolute bottom-20 left-1/2 -translate-x-1/2 bg-black/90 p-4 rounded-xl border border-yellow-500/50 w-72 backdrop-blur z-30 animate-in slide-in-from-bottom-5">
