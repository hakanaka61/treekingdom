...
// ==========================================
// 1. AYARLAR & OYUN DENGESİ (V22.0 FINAL)
// ==========================================
const CONFIG = {
  ...
  // BİNA AYARLARI (GÜVENLİ LİSTE)
  BUILDINGS: {
      house: { name: "Köy Evi", cost: 100, res: 'wood', score: 50, desc: "Nüfus Limitini +5 Artırır.", scale: 1.3 },
      tower: { name: "Gözetleme Kulesi", cost: 300, res: 'wood', score: 200, desc: "Her 10sn'de +10 Altın.", scale: 1.4 },
      castle: { name: "Büyük Kale", cost: 1000, res: 'wood', score: 5000, desc: "Merkez Üs.", scale: 1.6 }
  ...
  // GÖRSEL BOYUTLANDIRMA (DEVLEŞTİRME)
  SCALE_FACTORS: {
      king: 1.8,    // Kral ÇOK BÜYÜK
      castle: 1.6,  // Kale BÜYÜK
      house: 1.4,   // Ev belirgin
      tower: 1.5,   // Kule belirgin
      worker: 0.9,  // İşçi standart
      tree: 1.1,    // Ağaç dolgun
      deer: 0.8,    
      chest: 1.0,   
      stone: 1.0,
      gold: 1.0
...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    const savedUid = localStorage.getItem("orman_v22_uid");
      ...
      try {
          ...
          if (snapshot.exists()) {
              ...
              if (foundData && foundData.player.pin === pinInput) {
                  localStorage.setItem("orman_v22_uid", foundUid!);
              ...
              localStorage.setItem("orman_v22_uid", newUid); setLoginModal(false); initWorld(newUid);
          }
  ...
  const handleBuild = (e: any) => {
      ...
      const iy = Math.round((adjY/(CONFIG.TILE_HEIGHT*gs.current.camera.zoom/2) - adjX/(CONFIG.TILE_WIDTH*gs.current.camera.zoom/2)) / 2);
      const ix = Math.round((adjY/(CONFIG.TILE_HEIGHT*gs.current.camera.zoom/2) + adjX/(CONFIG.TILE_WIDTH*gs.current.camera.zoom/2)) / 2);
      const building = CONFIG.BUILDINGS[buildMode];
