...
// ==========================================
// 1. AYARLAR & OYUN DENGESİ (V31.0 FINAL)
// ==========================================
const CONFIG = {
...
export default function GamePage() {
  ...
  // --- AUTO-REPAIR SİSTEMİ (TİP GÜVENLİ) ---
  ...
  const updateLogic = () => {
      ...
      const isNight = cycle > (1 - CONFIG.NIGHT_DURATION_RATIO); 
          ...
          if(ent.owner === gs.current.userId && ent.type !== 'tree' && ent.type !== 'stone') {
              ...
              for(let fx = Math.floor(ent.pos.x - range); fx <= Math.floor(ent.pos.x + range); fx++) {
                  for(let fy = Math.floor(ent.pos.y - range); fy <= Math.floor(ent.pos.y + range); fy++) {
                      if(fx>=0 && fx<CONFIG.MAP_SIZE && fy>=0 && fy<CONFIG.MAP_SIZE && gs.current.fog[fx]) gs.current.fog[fx][fy] = true;
                  }
              }
          }
          ...
          if(ent.type === 'barbarian') {
              ...
              if(target && minD < 1) { 
                  if(Math.random() < 0.05 && !gs.current.shieldActive) { 
                      ...
                      spawnParticle(target.pos.x, target.pos.y, "red");
                      if(target.hp <= 0) {
                          target.dead = true;
  ...
  const render = () => {
    ...
    for(let x=0; x<CONFIG.MAP_SIZE; x++) { for(let y=0; y<CONFIG.MAP_SIZE; y++) { 
        ...
        if(gs.current.fog[x] && !gs.current.fog[x][y]) { ctx.fillStyle = '#000'; ctx.beginPath(); ctx.moveTo(pos.x, pos.y); ctx.lineTo(pos.x+CONFIG.TILE_WIDTH*zoom/2, pos.y+CONFIG.TILE_HEIGHT*zoom/2); ctx.lineTo(pos.x, pos.y+CONFIG.TILE_HEIGHT*zoom); ctx.lineTo(pos.x-CONFIG.TILE_WIDTH*zoom/2, pos.y+CONFIG.TILE_HEIGHT*zoom/2); ctx.fill(); continue; }
    ...
    gs.current.entities.forEach(ent => {
        if(ent.dead) return;
        const ix = Math.floor(ent.pos.x); const iy = Math.floor(ent.pos.y);
