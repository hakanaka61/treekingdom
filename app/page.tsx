...
const CONFIG = {
  ...
  // VARLIK BOYUTLARI (SCALING) - İSTEĞE GÖRE AYARLANDI
  SCALE_FACTORS: {
      king: 1.6,    // Kral çok büyük
      castle: 1.5,  // Kale büyük
      house: 1.3,   // Evler belirgin
      tower: 1.4,   // Kule (Yeni)
      worker: 0.9,  // İşçiler standart
      tree: 1.1,    // Ağaçlar dolgun
      deer: 0.8,    // Geyikler normal
      chest: 1.0,   // Sandık
      stone: 1.0,
      gold: 1.0
  ...
  UPGRADES: {
      tool: { name: "Elmas Uçlar", icon: "⚒️", desc: "Toplama hızını artırır.", baseCost: 150, mult: 1.6, effectDesc: (lvl: number) => `Süre: -${(lvl * 0.8).toFixed(1)}sn` },
      nature: { name: "Doğa Çağrısı", icon: "🌱", desc: "Spawn hızını artırır.", baseCost: 300, mult: 1.7, effectDesc: (lvl: number) => `Hız: %${(lvl * 10)} Artış` },
      speed: { name: "Hermes Çizmesi", icon: "👟", desc: "Yürüme hızı.", baseCost: 100, mult: 1.4, effectDesc: (lvl: number) => `Hız: +%${lvl * 10}` },
      cap: { name: "Şehir Yasası", icon: "📜", desc: "Nüfus limiti.", baseCost: 500, mult: 2.0, effectDesc: (lvl: number) => `Nüfus: +${(lvl+1) * 5}` }
  },
...
export default function GamePage() {
  ...
  const [ui, setUi] = useState({ 
      res: gs.current.player.resources, pop: 0, maxPop: 5, nextSpawn: 30, mana: 100, quest: gs.current.player.quest 
  });
  ...
  const [infoText, setInfoText] = useState("İmparatorluğunu Kur!"); // YENİ: Bilgi Kutusu
  ...
  useEffect(() => {
    ...
    const savedUid = localStorage.getItem("orman_v20_uid");
    if (savedUid) {
      ...
      try {
          ...
          if (snapshot.exists()) {
              ...
              if (foundData && foundData.player.pin === pinInput) {
                  localStorage.setItem("orman_v20_uid", foundUid!);
              ...
              localStorage.setItem("orman_v20_uid", newUid);
              setLoginModal(false);
              initWorld(newUid);
  ...
  const tradeResources = () => {
      if(gs.current.player.resources.wood >= 100) {
          gs.current.player.resources.wood -= 100;
          gs.current.player.resources.gold += 10;
          spawnFloatingText(0,0, "-100 ODUN / +10 ALTIN", "#facc15");
          updateUi();
      } else {
          setInfoText("Yetersiz Odun! (100 Odun = 10 Altın)");
      }
  };
  ...
  const castSpell = (spell: 'speed' | 'nature') => {
      ...
      if(spell === 'speed') {
          ...
          } else setInfoText("Yetersiz Mana (50 Gerekli)");
      }
      if(spell === 'nature') {
          if(p.mana >= 80) {
              p.mana -= 80;
          ...
          } else setInfoText("Yetersiz Mana (80 Gerekli)");
      }
      updateUi();
  };
  ...
  const saveGame = () => {
      ...
      gs.current.entities.forEach(e => {
          if (e.owner === gs.current.userId) {
              ...
              if (e.type === 'tower') score += 200; // Kule puanı
  ...
  const updateLogic = () => {
      ...
      if(now - gs.current.lastIncomeTime > 10000) {
          ...
          if(towers > 0) {
              const income = towers * 10;
              gs.current.player.resources.gold += income;
              spawnFloatingText(0, 0, `+${income} Vergi`, '#facc15');
              updateUi();
          }
  ...
  const handleInput = (e: any) => {
    ...
    if(e.type==='mousedown' || e.type==='touchstart') {
        ...
        gs.current.input.canClick = true; 
        ...
        if(e.type === 'touchmove' || e.buttons === 1) {
             ...
             if(dist > 10) { 
                 ...
                 gs.current.input.canClick = false; 
             }
             if(gs.current.input.isDragging) {
                 gs.current.camera.targetX += (gs.current.input.lastX - cx) * 1.5; 
                 gs.current.camera.targetY += (gs.current.input.lastY - cy) * 1.5;
             }
             gs.current.input.lastX = cx; gs.current.input.lastY = cy;
        }
    } 
        ...
        if(gs.current.input.canClick && !buildMode) {
             ...
             const mcx = cx - rect.left;
             const mcy = cy - rect.top;
             ...
             if(!clickedEnt) {
                 ...
                 if(king) { king.state = 'MOVE'; king.targetPos = {x:ix, y:iy}; king.targetId = null; setInfoText("Kral Yürüyor..."); }
             }
        } else if(buildMode && gs.current.input.canClick) {
             handleBuild({ clientX: cx, clientY: cy });
        }
        gs.current.input.isDragging = false;
    }
  };
  ...
  const handleBuild = (e: any) => {
      ...
      let cost = 0;
      if(buildMode === 'house') cost = 100;
      if(buildMode === 'castle') cost = 500;
      if(buildMode === 'tower') cost = 300;
      ...
      if(gs.current.player.resources.wood >= cost) {
          ...
          setInfoText("İnşaat Tamamlandı!");
          spawnFloatingText(ix*CONFIG.TILE_WIDTH/2 - iy*CONFIG.TILE_WIDTH/2, (ix+iy)*CONFIG.TILE_HEIGHT/2, `-${cost} Odun`, 'red');
      } else { setInfoText(`Yetersiz Kaynak (${cost} Odun)`); }
  };

  const spawnUnit = (type: 'worker' | 'king') => {
      if(ui.pop >= ui.maxPop) { setInfoText("Nüfus Dolu! Tekno'dan Artır."); return; }
      ...
      if(type==='king' && gs.current.entities.find(e => e.type==='king' && e.owner === gs.current.userId)) {
          setInfoText("Zaten bir Kralın var!"); return;
      }
  ...
  const render = () => {
    ...
    gs.current.entities.forEach(ent => {
        ...
        if(img && img.complete) {
            // SCALING (BOYUTLANDIRMA) UYGULANIYOR
            const scale = CONFIG.SCALE_FACTORS[ent.type] || 1.0;
            const w = CONFIG.TILE_WIDTH * zoom * scale;
            const h = (img.height/img.width) * w;
            const drawY = pos.y - h + (CONFIG.TILE_HEIGHT * zoom * 0.9); // Hizalama
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 text-white select-none overflow-hidden touch-none font-sans touch-none">
            ...
            <div className="bg-slate-900 rounded-2xl border border-emerald-500/30 w-full max-w-md p-6 shadow-2xl animate-in fade-in zoom-in">
                ...
                <div className="space-y-3 text-sm text-gray-300">
                    <div className="flex items-center gap-3 bg-black/40 p-2 rounded">
                        ...
                        <p><strong>İşçiler:</strong> 60 ET karşılığında üretilir. Otomatik kaynak toplarlar.</p>
                    </div>
                    <div className="flex items-center gap-3 bg-black/40 p-2 rounded">
                        <span className="text-2xl">🦌</span>
                        <p><strong>Avcılık:</strong> Geyik avlayarak ET topla. Ormanda bolca bulunurlar.</p>
                    </div>
                    <div className="flex items-center gap-3 bg-black/40 p-2 rounded">
                        <span className="text-2xl">🤴</span>
                        <p><strong>Kral:</strong> 500 Altın ile gelir. İşçilere HIZ aurası verir. Tıklayarak yönet.</p>
                    </div>
                    <div className="flex items-center gap-3 bg-black/40 p-2 rounded">
                        <span className="text-2xl">🗼</span>
                        <p><strong>Kule:</strong> 300 Odun. Her 10 saniyede +10 Altın kazandırır.</p>
                    </div>
                </div>
                <button onClick={()=>setShowTutorial(false)} className="w-full mt-6 bg-emerald-600 py-3 rounded-lg font-bold hover:bg-emerald-500 transition shadow-lg">BAŞLA 🚀</button>
            </div>
        </div>
      )}
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 pointer-events-auto z-20 w-full px-2">
          ...
          {/* AKILLI BİLGİ PANELİ (YENİ) */}
          <div className="bg-black/80 px-4 py-2 rounded-lg border border-yellow-500/30 text-xs text-yellow-100 font-bold mb-1 shadow-lg text-center min-w-[200px]">
              {infoText}
          </div>

          <div className="flex gap-2 w-full max-w-md justify-center mb-1">
          ...
          <div className="bg-black/90 p-2 rounded-2xl border border-yellow-500/20 flex gap-1 sm:gap-2 backdrop-blur-sm shadow-2xl overflow-x-auto max-w-full">
            <Btn i="⚡" l="HIZ" sub="50 Mana" onClick={()=>{castSpell('speed'); setInfoText("Birimler 10sn Hızlandı!");}} desc="İşçiler hızlanır" />
            <Btn i="⚖️" l="PAZAR" sub="Takas" onClick={()=>{tradeResources();}} desc="100 Odun -> 10 Altın" />
            <div className="w-px bg-white/20 mx-1"></div>
            <Btn i="👷" l="İşçi" sub="60 Et" onClick={()=>{spawnUnit('worker'); setInfoText("Yeni İşçi Geliyor...");}} desc="Kaynak toplar" />
            <Btn i="🤴" l="Kral" sub="500 Altın" onClick={()=>{spawnUnit('king'); setInfoText("Kral Sahaya İndi!");}} desc="Hız Aurası" />
            <div className="w-px bg-white/20 mx-1"></div>
            <Btn i="🏠" l="Ev" sub="100 Odun" onClick={()=>{setBuildMode('house'); setInfoText("Haritaya dokun ve Ev yap.");}} act={buildMode==='house'} desc="Dekor" />
            <Btn i="🗼" l="Kule" sub="300 Odun" onClick={()=>{setBuildMode('tower'); setInfoText("Haritaya dokun ve Kule dik.");}} act={buildMode==='tower'} desc="Pasif Altın" />
            <Btn i="🧪" l="Tekno" sub="Yükselt" onClick={()=>{setTechModal(true); setInfoText("Teknoloji Geliştir");}} act={techModal} desc="Gelişim" />
          </div>
      </div>
